...
<%--
		1. CSS
			...
			=> Selector
				=> 기본
					= 태그명
					  ...
					  document.querySelector("태그명"); => 일반JavaScript
					  new Vue({
					    el:'태그명'
					  })
					  많이 사용되지 않음 => 태그가 많다 => ID로 구분하자
					===================구분자====================  
					= ID명 => 중복이 없는 구분자
					  #ID명{
					    속성:값 
					  }
					  $('#ID명').change(function(){처리문장}) => Jquery
			...
			태그명[속성*=값]  => contains => 크롤링
			  ex) img[src*=/recipe/] => <img src="값"> 값에 /recipe/를 포함하는것을 들고와라
			태그명[속성$=값]  => endsWith => 이미지만 가지고 오고 싶은 경우
						   => 후기 게시판
		      ex) img[src$='jpg|png|gif']
		      		=> $('img[src$="jpg|png|gif"]')
		    ...
		    *** HTML,XML => 계층구조 (트리형태)
		  	...
		  	인접 => div > h1+span => th+td => th옆에있는 td태그 까지만
		  		   div > h1~span => h1부터 span안에있는 모든태그 까지
		  	=======================================================
		  	반응 선택자 
		  		ID명, Class명, 태그명 : hover	
		  				      		 ====== => mouseover
		  				      		 
		  	구조 선택자 : nth-child (수열) (n은 0부터!)
		  		nth-child(2n) ==> n=0 => 0 2 4 6 ...
		  	...
		  	192page => 위치속성
		    -----------------------------------
		   	static : 태그에 맞춰서 배치 (순서대로 배치 => default
		    -----------------------------------
		    relative : 상대태그좌표를 이용 (옆에 있는 태그가 기준점)
		    -----------------------------------
		    absolute : 절대좌표 이용 
		    -----------------------------------
		    fixed : 고정 => 상세보기
